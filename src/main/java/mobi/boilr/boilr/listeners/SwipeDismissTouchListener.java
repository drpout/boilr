		...
		switch (motionEvent.getActionMasked()) {
			...
			case MotionEvent.ACTION_UP: {
				...
				if (dismiss) {
					...
					// cancel
					mView.animate().translationX(0).alpha(1).setDuration(mAnimationTime)
					.setListener(null);
				}
			...
			case MotionEvent.ACTION_MOVE: {
				...
				if (Math.abs(deltaX) > mSlop && Math.abs(deltaY) < Math.abs(deltaX) / 2) {
					...
					cancelEvent
					.setAction(MotionEvent.ACTION_CANCEL | (motionEvent.getActionIndex() << MotionEvent.ACTION_POINTER_INDEX_SHIFT));
